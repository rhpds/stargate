...
def collect_namespace_state(fixture_dir: Path) -> List[CollectedEvidence]:
    ...
    Handles both individual resources and list resources (kind: List,
    kind: PodList, etc.) from oc get -o json output.
    """
    results = []
        ...
        try:
            ...
            if kind == "List" or kind.endswith("List"):
                ...
                if kind == "List" and items:
                    ...
                    if first_kind in ("Pod",):
                        results.append(collect_pods(data))
                        continue
                    for item in items:
                        try:
                            ev = collect_from_data(item)
                            ev.source = f"file:{path.name}"
                            results.append(ev)
                        except ValueError:
                            continue
                else:
