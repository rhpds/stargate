...
│                                                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────┐ │
│  │Collectors│→ │Evaluator │→ │ Bundler  │→ │ Events │ │
│  │          │  │(Rubrics) │  │(History) │  │  Bus   │ │
│  └────┬─────┘  └──────────┘  └────┬─────┘  └───┬────┘ │
│       │                           │             │       │
│  ┌────┴─────────────────────┐  ┌──┴──┐    ┌────┴────┐ │
...
**Build**:
- **Event types**:
  - `evaluation.completed` — run evaluated, includes outcome and failure classes
  - `failure.classified` — new failure detected with class and recommended remediation
  - `failure.unclassified` — failure that doesn't match any rubric class (LLM candidate)
  - `remediation.proposed` — AI proposal generated
  - `environment.degraded` — previously passing environment now failing (drift)
