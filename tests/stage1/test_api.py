...
class TestRunEndpoints:
    ...
    def test_get_run(self, client):
        ...
        resp = client.get("/runs/get-run-001")
...
class TestStageEndpoints:
    ...
    def test_start_stage_promotes_run_to_running(self, client):
        ...
        resp = client.get("/runs/stage-test-run")
    ...
    def test_start_stage_run_not_found(self, client):
        resp = client.post("/runs/nonexistent/stages/ns-ready/start")
        assert resp.status_code == 404

    def test_submit_evidence(self, client):
        self._create_run(client)
        client.post("/runs/stage-test-run/stages/namespace-ready/start")
        resp = client.post("/runs/stage-test-run/stages/namespace-ready/evidence", json={
            "type": "fixture",
            "source": "test",
    ...
    def test_submit_evidence_stage_not_found(self, client):
        self._create_run(client)
        resp = client.post("/runs/stage-test-run/stages/nonexistent/evidence", json={
            "type": "fixture",
            "source": "test",
            "observed": {},
            "result": "pass",
    ...
    def test_evaluate_stage_with_inline_evidence(self, client):
        self._create_run(client)
        client.post("/runs/stage-test-run/stages/namespace-ready/start")
        resp = client.post("/runs/stage-test-run/stages/namespace-ready/evaluate", json={
    ...
    def test_complete_stage(self, client):
        self._create_run(client)
        client.post("/runs/stage-test-run/stages/namespace-ready/start")
        client.post("/runs/stage-test-run/stages/namespace-ready/evaluate", json={
            "evidence": {"namespace_exists": True},
        })
        resp = client.post("/runs/stage-test-run/stages/namespace-ready/complete")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_submit_evidence_after_completion_blocked(self, client):
        ...
        client.post("/runs/stage-test-run/stages/namespace-ready/complete")
        resp = client.post("/runs/stage-test-run/stages/namespace-ready/evidence", json={
    ...
    def test_evaluate_updates_stage_status(self, client):
        ...
        resp = client.get("/runs/stage-test-run/report")
...
class TestRubricEndpoints:
    def test_validate_rubric_valid(self, client):
        resp = client.post("/rubrics/validate", json={
            "rubric": {
                "id": "test-rubric",
                "version": "v0.1.0",
                "stage": "test-stage",
    ...
    def test_get_rubric(self, client):
        resp = client.get("/rubrics/namespace-ready")
        assert resp.status_code == 200
        data = resp.json()
        assert data["id"] == "namespace-ready"

    def test_get_rubric_not_found(self, client):
        resp = client.get("/rubrics/nonexistent-stage")
        assert resp.status_code == 404

    def test_evaluate_rubric_directly(self, client):
        resp = client.post("/rubrics/namespace-ready/evaluate", json={
            "evidence": {"namespace_exists": True},
        })
        assert resp.status_code == 200
...
class TestReportEndpoints:
    def _setup_run_with_stages(self, client):
        ...
        client.post("/runs/report-run/stages/namespace-ready/evidence", json={
            "type": "fixture",
            "source": "test",
            "observed": {"namespace_exists": True},
            "result": "pass",
        ...
        client.post("/runs/report-run/stages/namespace-ready/complete")
    ...
    def test_bottlenecks(self, client):
        self._setup_run_with_stages(client)
        resp = client.get("/runs/report-run/bottlenecks")
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 1
...
class TestAIProposalEndpoints:
    def _setup_failed_run(self, client):
        ...
        client.post("/runs/ai-run/stages/namespace-ready/start")
        client.post("/runs/ai-run/stages/namespace-ready/evidence", json={
            "type": "fixture",
            "source": "test",
            "observed": {"namespace_exists": False},
            "result": "fail",
        })
        client.post("/runs/ai-run/stages/namespace-ready/evaluate", json={
            "evidence": {"namespace_exists": False},
        })
        client.post("/runs/ai-run/stages/namespace-ready/complete")

    def test_failure_summary(self, client):
        self._setup_failed_run(client)
        resp = client.post("/runs/ai-run/proposals/summary")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_failure_summary_not_found(self, client):
        resp = client.post("/runs/nonexistent/proposals/summary")
        assert resp.status_code == 404

    def test_pr_text_generation(self, client):
        self._setup_failed_run(client)
        resp = client.post("/runs/ai-run/proposals/pr-text")
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "proposed"
        assert data["approved"] is False
    ...
    def test_rubric_diffs(self, client):
        self._setup_failed_run(client)
        resp = client.post("/runs/ai-run/proposals/rubric-diffs")
        assert resp.status_code == 200
        data = resp.json()
