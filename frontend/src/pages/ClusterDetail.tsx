...
import { useClusterNodes, useClusterFailures, useClusterNamespaces, useEvents } from '../api/hooks';
...
function MetricCard({ label, value, sub, onClick }: { label: string; value: string | number; sub?: string; onClick?: () => void }) {
  return (
...
function NamespaceTable({ namespaces, navigate }: { namespaces: ClusterNamespace[]; navigate: (path: string) => void }) {
  if (namespaces.length === 0) return <p className="text-[#6A6E73] text-sm">No namespaces found.</p>;
  return (
    <div className="space-y-0.5">
      <div className="grid grid-cols-[1fr_80px_80px_80px_80px_150px_100px] gap-3 text-xs text-[#6A6E73] uppercase tracking-wider font-bold pb-2 border-b border-[#2e2e2e]">
        <span>Namespace</span><span className="text-right">Evals</span><span className="text-right">Passed</span><span className="text-right">Failed</span><span className="text-right">Health</span><span>Top Failure</span><span>Last Seen</span>
      </div>
      {namespaces.map((ns) => (
        <div
          key={ns.namespace}
          onClick={() => navigate(`/lab/${encodeURIComponent(ns.namespace)}`)}
          className={`grid grid-cols-[1fr_80px_80px_80px_80px_150px_100px] gap-3 items-center py-1.5 rounded cursor-pointer hover:bg-[#1e1e1e] transition ${ns.is_ecosystem ? 'border-l-2 border-l-[#EE0000]' : 'opacity-50'}`}
        >
          <span className="text-sm text-[#73BCF7] font-medium truncate">
            {ns.namespace}
          ...
          <span className="text-right"><span className="text-[10px] font-semibold px-1.5 py-0.5 rounded" style={{ backgroundColor: ns.health_rate >= 80 ? '#3E8635' : ns.health_rate >= 50 ? '#F0AB00' : '#C9190B', color: '#fff' }}>{Math.round(ns.health_rate)}%</span></span>
          <Link to={`/failures?selected=${encodeURIComponent(ns.top_failure || '')}`} onClick={(e) => e.stopPropagation()} className="text-xs text-[#8A8D90] truncate hover:text-[#73BCF7]" title={ns.top_failure || ''}>{ns.top_failure || '--'}</Link>
...
function FailureDistribution({ failures, navigate }: { failures: Record<string, number>; navigate: (path: string) => void }) {
  const entries = Object.entries(failures).sort(([, a], [, b]) => b - a);
  if (entries.length === 0) return <p className="text-[#6A6E73] text-sm">No failures recorded.</p>;
...
export default function ClusterDetail() {
  ...
  const events = useEvents({ cluster: name || '' });
  ...
  const eventList = (events.data || []) as Array<{ event_id: string; event_type: string; lab_code: string; failure_class: string; outcome: string; message: string; priority: number; timestamp: string }>;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 space-y-6">
      <div>
        ...
        <span className="text-[#6A6E73] text-sm">Cluster</span>
        <h1 className="text-3xl font-bold text-white mt-2" style={{ fontFamily: 'Red Hat Display' }}>{name}</h1>
        <p className="text-[#6A6E73]">Cluster overview and namespace breakdown</p>
      </div>

      {isLoading ? (
        <div className="animate-pulse space-y-4">
          <div className="grid grid-cols-6 gap-4">{[1,2,3,4,5,6].map(i => <div key={i} className="bg-[#212121] rounded-lg h-20" />)}</div>
        </div>
      ) : (
        <>
          {n && (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              <MetricCard label="Failing" value={failingCount} sub={`${totalFailures} total failures`} onClick={() => navigate('/failures')} />
            </div>
          )}
          ...
          {eventList.length > 0 && (
            <section>
              <SectionHeader>Recent Events ({eventList.length})</SectionHeader>
              <div className="bg-[#212121] border border-[#2e2e2e] rounded-lg p-4 space-y-1">
                {eventList.slice(-10).reverse().map((ev) => (
                  <div key={ev.event_id} className="flex items-center gap-3 text-sm py-1">
                    <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: ev.priority >= 7 ? '#C9190B' : ev.priority >= 4 ? '#F0AB00' : '#3E8635' }} />
                    <span className="text-[#6A6E73] text-xs w-20 shrink-0">{relativeTime(ev.timestamp)}</span>
                    <Link to={`/lab/${encodeURIComponent(ev.lab_code)}`} className="text-[#73BCF7] text-xs hover:underline w-32 truncate shrink-0">{ev.lab_code || '--'}</Link>
                    <span className="text-xs px-1.5 py-0.5 rounded shrink-0" style={{ backgroundColor: ev.outcome === 'fail' ? '#C9190B' : ev.outcome === 'warn' ? '#F0AB00' : '#3E8635', color: '#fff' }}>{ev.outcome}</span>
                    {ev.failure_class && (
                      <Link to={`/failures?selected=${encodeURIComponent(ev.failure_class)}`} className="text-xs text-[#8A8D90] hover:text-[#73BCF7] truncate">{ev.failure_class}</Link>
                    )}
                    <span className="text-xs text-[#6A6E73] truncate flex-1">{ev.message?.slice(0, 80)}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          <section>
            ...
            <div className="bg-[#212121] border border-[#2e2e2e] rounded-lg p-4 overflow-x-auto">
              <NamespaceTable namespaces={namespaces} navigate={navigate} />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="bg-[#212121] border border-[#2e2e2e] rounded-lg p-4">
              <FailureDistribution failures={failureMap} navigate={navigate} />
            </div>
          </section>
        </>
      )}
